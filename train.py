...
def get_dataset(optsDataset):
    if optsDataset == 'mnist':
        return load_mnist(opts.batch_size)
    if optsDataset == 'small_norb':
        return load_small_norb(opts.batch_size)
    if optsDataset == 'cifar10':
...
def main(opts):
    ...
    train_loader, valid_loader, test_loader = get_dataset(opts.dataset)
